# Twitter Thread - Scout v1.1.0

---

**Tweet 1**

Scout v1.1.0 is live. Ninja Spinner is one week in with 62 tournaments tracked. Here's what's new and what the data is showing.

scout.trainerlab.io

---

**Tweet 2**

Meowth ex has a +23 point winning edge. It shows up in 52% of 1st-place decks but only 29% of the overall field. Dragapult Meowth leads at 10.2% meta share and climbing.

Format Edge surfaces exactly these signals. scout.trainerlab.io/ninja-spinner/card-analysis

---

**Tweet 3**

Dusknoir Greninja is the fastest-growing archetype in Ninja Spinner with a +12.1% trend delta. Night Stretcher (-24%) and Boss's Orders (-24%) are dropping hard. Chaos Rising is rewriting the staple base.

---

**Tweet 4**

Card detail pages now show the winning edge broken down by archetype. You can see exactly which archetypes a card overperforms in, not just whether it's popular, but whether it's winning.

---

**Tweet 5**

Coming soon: auto-generated weekly meta reports, an API layer, and tournament prep tools.

Scout is building toward giving every player access to the kind of data analysis that used to require hours of manual work.

---

**Tweet 6**

Scout tracks JP City League results across formats, updated daily. Free, no login.

scout.trainerlab.io
